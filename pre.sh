...
echo -e "———————————————————————————
   \033[32mKangle版本选择\033[0m
   （要想使用kangle最新商业版请使用64位操作系统）
1. 极速安装 Kangle 3.5.10 经典版(推荐)
2. 编译安装 Kangle 3.5.21 开发版
3. 极速安装 Kangle 3.4.8 经典版"
read -p "请输入序号并回车:" YORN
if [ "$YORN" = "2" ]; then
kangle_var="$KANGLE_VERSION";
kangle_completed="0";
...
if [ "${isC7}" == "" ];then
	...
	wget -q $DOWNLOAD_FILE_URL/repo/mysql-community.repo?v=1002 -O /etc/yum.repos.d/mysql-community.repo
else
	yum_repos_s=`ls /etc/yum.repos.d | wc -l`;
	if [ "$yum_repos_s" == '0' ]; then
...
yum -y install yum-fastestmirror epel-release nss curl libcurl
#yum -y update
